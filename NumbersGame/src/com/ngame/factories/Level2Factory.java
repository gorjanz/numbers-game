...
/**
 * Class used to load levels marked with difficulty 2
 * @author Gorjan
 *
 */
public class Level2Factory extends LevelFactory {

	private static final String TAG = "Level2Factory";
	
	private ArrayList<Level> levels;
	private Context ctx;

	public Level2Factory(Context c) {
		ctx = c;
		Level newLvl;
		levels = new ArrayList<>();
		try {
			
			InputStream is = ctx.getResources().getAssets().open("levels/level2.txt");
			...
			e.printStackTrace();
			Log.e(TAG, "Error in reading levels from level2.txt");
		}
	}
